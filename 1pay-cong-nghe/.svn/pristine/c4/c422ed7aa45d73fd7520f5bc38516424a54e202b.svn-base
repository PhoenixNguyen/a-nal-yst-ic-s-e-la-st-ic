						...
						<div class="right_content">
							<h1 class="srv_title">
								...
								<div>
									<%if(account.checkRole(Account.ACCOUNT_STAFF_ROLE)){ %>
										<div id="dash_tab">
											<a href="<%=request.getContextPath() %>/protected/merchant_profile_manager.html" class="tab_slc"><span>All(${model.total_all})</span></a>
											<a href="<%=request.getContextPath() %>/protected/merchant_profile_approved.html" class="tab"><span>Effect(${model.total_approved})</span></a>
											
											<a href="<%=request.getContextPath() %>/protected/merchant_profile_active.html" class="tab"><span>Processing(${model.total_active})</span></a>
											<a href="<%=request.getContextPath() %>/protected/merchant_profile_reject.html" class="tab"><span>Canceled(${model.total_reject})</span></a>
											<!-- 
											...
											<c:if test="${model.isProposeProfile}">
												<a href="<%=request.getContextPath() %>/protected/merchant_profile_upgrate_user.html" class="addcomparison_link"><span>Profile recommend</span></a>
											</c:if>
											<div class="clear"></div>
										</div>
									<%}%>
				                 	...
				                 	<div class="srv_row">
										...
										<input type="hidden" value="<%=request.getContextPath() %>${model.path_identity_image}" id="path_identity_image"/>
					                 	...
					                 	<c:if test="${model.total>0 }">
					                 		<span class="pagebanner"> ${model.total} results found, from ${model.offset + 1} to ${(model.offset + model.pagesize) > model.total ? model.total : (model.offset + model.pagesize) }. </span>
					                 	</c:if>
										<display:table name="model.list" id="list" 
										    ...
										    <display:column title="Payment period" headerClass="transhead " class="transdata width70" style="border: 1px solid #CCC;text-align:center;">
										    ...
										    <display:column title="Actions" headerClass="transhead width80" class="svr_data1" style="border: 1px solid #CCC;">
										    	<a href="merchant_profile_detail.html?id=${list.id}&call_back_url=${model.url_callback_full}" rel="leanmodal" class="act_detail" title="see Merchant Profile"></a>
