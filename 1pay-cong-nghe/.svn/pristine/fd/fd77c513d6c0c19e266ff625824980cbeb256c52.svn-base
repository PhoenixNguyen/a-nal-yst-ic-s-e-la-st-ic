								...
								<form action="manager.wss" method="post">
									<div class="dash_row">
										...
										<div id="dash_tab" style="padding-right: 0;">
											...
											</a> <a href="manager.wss?tab=active" class="tab${param.tab=='active'?'_slc':''}"><span>Đã được ${okLabel} (<fmt:formatNumber var="totalActive" value="${model.totalActive}" currencyCode="vnd" /> <c:out value="${totalActive}" />)</span>
											...
											<a href="<%=request.getContextPath()%>/console/create.wss" class="addproduct_link"><span>Tạo mới sản phẩm</span></a>
											<c:if test="${account_logined.isBizManager() || account_logined.isOperator() || account_logined.isBizSupporter()}">
												<a href="<%=request.getContextPath() %>/console/sms-command-code-manager.wss" class="sms_link" style="margin-right: 5px;"><span>Quản lý mã SMS</span></a>
											</c:if>	
											<div class="clear"></div>
										</div>
									</div>
									...
									<div class="box_locketqua">
										...
										<div class="filter_row">
											...
											<div class="product_slc_filter filter_row">
												<fieldset>
													<legend>
														<input type="checkbox" name="csChkAll" value="csChkAll" id="csChkAll" class="chck_filter" /> <strong>Trạng thái dịch vụ thanh toán</strong>
														<script type="text/javascript">
															$(document).ready(function(){
													...
													<div style="padding: 2px;">
														...
														<c:forEach var="status" items="${model.allChargingStatus }">
															...
															<div style="display: inline-block;">
																...
																<label class="lbl_chcksub charging_status_${status.key}">${fn:replace(status.value, '1PAY', '<span style="color:#2c8f39;">1PAY</span>')}</label>
															</div>
															</c:if>
														</c:forEach>
													</div>
									...
									<div class="srv_row">
										...
										<display:table name="model.list" id="list" 
										    ...
										    <display:column 
										    	title="Đăng ký dịch vụ thanh toán <img id='csTip' src='${pageContext.request.contextPath}/images/question.png' alt='?' width='18' style='margin-top:-10px' align='middle' />" 
												...
												<a class="more_normal" title="Đăng ký dịch vụ" href="#"></a>
										    </display:column>
										    <display:column 
										    	title="Merchant ON/OFF charging <img src='${pageContext.request.contextPath}/images/question.png' alt='?' title='Merchant hoàn toàn chủ động Bật/tắt Dịch vụ thanh toán cho Sản phẩm' width='18' style='margin-top:-10px' align='middle' />" 
										    	headerClass="transhead width90" 
										    	class="svr_data1"
										    	style="border: 1px solid #CCC;text-align:center;${!isOwnerProduct?'background:#F3F3F3;':'' }">
												<div title="${!isOwnerProduct?(list.merchant_action == APPROVED_STATUS?'':'Merchant off sản phẩm'):(list.staff_action == APPROVED_STATUS?'Sản phẩm đã được xác nhận':(list.staff_action == REJECT_STATUS?'Sản phẩm bị từ chối':''))}"
										    ...
										    <display:column 
										    	title="1PAY ON/OFF charging <img src='${pageContext.request.contextPath}/images/question.png' alt='?' title='1PAY Bật/ tắt dịch vụ thanh toán cho Sản phẩm' width='18' style='margin-top:-10px' align='middle' />" 
										    	headerClass="transhead width90" 
										    	class="svr_data1"
										    	style="border: 1px solid #CCC;text-align:center;${!isOperatorOrBizSupporter?'background:#F3F3F3;':'' }">
										    	<c:set var="logTitle" value="${model.isStaff?(list.merchant_action == APPROVED_STATUS?'':'Merchant off sản phẩm'):(list.staff_action == APPROVED_STATUS?'Sản phẩm đã được xác nhận':(list.staff_action == REJECT_STATUS?'Sản phẩm bị từ chối':''))}" />
												...
												$('.onoffswitch-checkbox').live('click', function() {
													...
													if('true' != '${model.isStaff}' || 'true' == $(this).attr('owner')) {
														...
														if(!haveActiveOrPendingCharg) {
															alert('Sản phẩm chưa đăng ký charging nào');
															return false;
														}
													} else {
														return false;
													...
													var alertMess = 'Bạn có chắc chắn ON/OFF sản phẩm?';
													...
													if(active == '${INIT_STATUS}' || active=='${REJECT_STATUS}' || active == '${PENDING_STATUS}') {
														alertMess = 'true' == '${model.isStaff}' ? 'Bạn có chắc chắn ON sản phẩm?' : 'Bạn có chắc chắn ON sản phẩm?';
													} else if(active == '${APPROVED_STATUS}') {
														alertMess = 'true' == '${model.isStaff}' ? 'Bạn có chắc chắn OFF sản phẩm?' : 'Bạn có chắc chắn OFF sản phẩm?';
													}
										...
										<script type="text/javascript">
											...
											$('.go_iac').live('click', function() {
												...
												if("leanModal" != $(this).attr('rel')) {
													alert("${model.isStaff?'Sản phẩm chưa đăng ký kích hoạt SMS Plus Charging':'Sản phẩm đã được kích hoạt SMS Plus Charging'}");
													return false;
												}
											});
										</script>
							...
							<c:otherwise>
								<div id="khoitao_box">
									...
									<div class="content">
										...
										<ul>
											<li>Quản trị <strong>sản phẩm</strong>
											</li>
											<li>Theo rõi thông tin, sản lượng từng sản phẩm</li>
											<li>Chia sẻ API, quản lý Teamwork.</li>
										</ul>
									</div>
									<div>
										<a class="btn_khoitaosp" href="<%=request.getContextPath()%>/console/create.wss">Khởi tạo sản phẩm</a>
									</div>
								</div>
							</c:otherwise>
						</c:choose>
