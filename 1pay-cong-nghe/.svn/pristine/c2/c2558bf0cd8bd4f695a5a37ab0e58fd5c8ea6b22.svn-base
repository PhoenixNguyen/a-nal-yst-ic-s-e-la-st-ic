...
<body>
								...
								<c:when test="${model.product != null}">
									<div class="srv_row">
										...
										<div class="product_dt_type">
											Product type: <strong><c:out value="${model.product.productCategory.title}" /></strong> <!-- <a href="#" class="link_edit">[Sửa]</a> -->
										</div>
										...
										<div>
											...
											<a href="<%=request.getContextPath() %>/download/androidSDK?access_key=${model.product.accessKey}"><img src ="<%=request.getContextPath() %>/images/btn_sdk.png"/></a>
										</div>
			                    		
									</div>
									<div class="srv_row">
										<strong>Payment integration information</strong>
											...
											<tr>
												<td>
												 ...
												 <td width="200" class="feature_box">
						                        	<a href="http://dev.1pay.vn" class="link_feature">Payment integration Documents (APIs)</a>
						                        	<a href="#referers_pop" class="link_feature" name="referers_pop" rel="leanModal">Access limitation</a>
						                        	<a href="#" class="link_feature" id="key_generator">Create new key</a>
									...
									<div class="srv_row">
										<strong>Note:</strong>
										<ul>
											<li>access_key: is an unique string for each product which is used to identify product when calling api.</li>
											<li>secret: is a string to make signature when calling api, merchant must keep secret this value.</li>
										</ul>
									</div>
		...
		<form action="<%=request.getContextPath()%>/console/ajax.wss"
			...
			<div class="pop_box">
				...
				<table cellpadding="5" cellspacing="0">
					<tbody>
						...
						<tr class="srv_row">
							<td class="profile_label" width="140">Product code</td>
							<td>
								<input name="productCode" id="productCode" class="txt_reglog" value="${model.product.productCode}" />
							</td>
						</tr>
						<tr class="srv_row">
							<td class="profile_label">Product category</td>
							<td>
						...
						<tr class="srv_row">
							<td class="profile_label">Description</td>
							<td>
			...
			<div>
				<input type="submit" name="submit" value="Update"
					...
					value="Reset" class="btn_graysmall modal_close" />
			</div>
		</form>
		<script type="text/javascript">
			$(document).ready(function() {
				$('form[name=product_post]').submit(function() {
						$.post($(this).attr('action'), $(this).serialize(), function(json) {
								if ($.isNumeric(json)) {
									var x = parseInt(json);
									if (x == 1) {
										alert('Successful update your product');
										...
										alert('Unsuccessful update your product');
									}
								} else {
									alert(json);
								}
	...
	<div id="referers_pop">
		<form action="<%=request.getContextPath()%>/console/ajax.wss"
			...
			<div class="pop_box">
				<div class="product_title">
					<h1>Update access limitation</h1> 
				</div>
				<table cellpadding="5" cellspacing="0">
					<tbody>
						<tr class="srv_row">
							<td class="profile_label" width="140">Access limitation</td>
							<td>
				...
				$('form[name=referers_post]').submit(function() {
						$.post($(this).attr('action'), $(this).serialize(), function(json) {
								if ($.isNumeric(json)) {
									var x = parseInt(json);
									if (x == 1) {
										alert('Successful update access limitation');
										
										$('.api_access_tbl #referers').html($('form[name=referers_post] input[name=referers]').val());
										
										$('.modal_close').click();
									} else {
										alert('Unsuccessful update access limitation');
									}
								} else {
									alert(json);
								}
