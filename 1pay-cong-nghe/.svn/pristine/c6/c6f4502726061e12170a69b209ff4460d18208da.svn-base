				...
				<div id="table_record">
					...
					<col width="6%"/>
						<tr>
							...
							<th>Kỳ</th>
							<th>Tài khoản</th> -->
							<c:set value="null" var="nl"/>
							<% 
								String ob = null;
								request.getSession().setAttribute("noValueInt", ob);
							%>
							<th>Sửa
						...
						<c:forEach items="${model.categoryMap}" var="item">
							<tag:treeTr treeMap="${item.value}"/>
						</c:forEach>
		...
		<script type="text/javascript">
				$(document).ready(function(){
					...
					$('.record_management_popup').live('click', function(){
						...
						$('#record_management_popup form[name=money_flow_record_save] select[name=parent_id]').html('');
					...
					$('.record_management_edit_popup').live('click', function(){
						...
						var parents = $('#record_management_popup form[name=money_flow_record_save] select[name=parent_id]');
						parents.html('');
						var values = $.map(options ,function(option) {
							if(option.className == type)
					...
					var options = $('#record_management_popup form[name=money_flow_record_save] select[name=parent_id] option');
					$('#record_management_popup form[name=money_flow_record_save] select[name=type]').live('click', function(){
						var select = $('#record_management_popup form[name=money_flow_record_save] select[name=type]');
						var type = select.val();
						...
						//remove
						//$('#record_management_popup form[name=money_flow_record_save] select[name=parent_id] option[class !='+ type +']').remove();
					});
