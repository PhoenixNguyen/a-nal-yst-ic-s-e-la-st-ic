				...
				<div id="w_body">
					<div class="row_sub">
						...
						<div class="right_content">
							...
							<div>
								<form id="smsReport" action="sms-report.html" method="post">
									<div id="dash_tab">
											<a id="cont" href="<%=request.getContextPath() %>/protected/card-report.html" class="tab"><span>Card Charing</span></a> 
											<a id="telco" href="<%=request.getContextPath() %>/protected/sms-report.html" class="tab_slc"><span>SMS Charing</span></a> 
											...
											<a href="<%=request.getContextPath()%>/protected/export_sms_report.html?${model.export_parameter}" class="export_link"><span>Export files</span></a>
											<div class="clear"></div>
									</div>
										...
										<div class="box_locketqua">
											<div class="filter_row">
												<div class="other_filter">
													<label class="lbl_dash_filter">Transaction status:</label> 
													<select name="status" class="dashboard_filter">
														...
														<option value="0" ${param.status == '0'?'selected="selected"':''}>Unsuccess</option>
													</select>
												</div>
												<div class="other_filter">
													<label class="lbl_dash_filter">Charging:</label> 
													<select name="billingStatus" class="dashboard_filter">
														<option value="" ${(param.billingStatus == null || param.billingStatus == '')?'selected="selected"':''}>All</option>
														<option value="1" ${param.billingStatus == '1'?'selected="selected"':''}>Charging</option>
														<option value="0" ${param.billingStatus == '0'?'selected="selected"':''}>Not charging</option>
													</select>
												</div>
												...
												<script type="text/javascript">
													$(document).ready(function() {
														$('#reservation').daterangepicker({
															...
															'First 15 days of month': [Date.today().moveToFirstDayOfMonth().add({ days: +15 }), Date.today().moveToLastDayOfMonth()],
							                ...
							                <div class="filter_row">
							                 	<span class="add-on">Command code:</span>
							               		<input type="text" name="commandCode" value="${param.commandCode}" class="txt_filter" />
							                </div>
							                <%} %>
											<div class="filter_row">
							                 	<span class="add-on">Message content:</span>
							               		<input type="text" name="moMessage" value="${param.moMessage}" class="txt_filter" />
							                </div>
							                <%if(account.checkRoles(new String[]{Account.ACCOUNT_OPERATION_MANAGER_ROLE, Account.ACCOUNT_BIZ_SUPPORTER_ROLE, Account.ACCOUNT_CUSTOMER_CARE_ROLE})) {%>
							                <div class="filter_row">
							                 	<span class="add-on">The returned content:</span>
											...
											<c:if test="${model.merchants != null && fn:length(model.merchants) >1 }">
												...
												<div class="filter_row">
													...
													<input id="searchMerchant" name="searchMerchant" value="${param.searchMerchant }" class="dashboard_filter" placeholder="Enter merchant accout"/>
													</div>
												</div>
											</c:if>
											<!-- .field-group -->
               				...
               				<div class="srv_row">
								...
								<display:table name="model.list" id="list" 
								    ...
								    <display:column title="First of" headerClass="transhead" class="transdata" property="shortCode" style="border: 1px solid #CCC;text-align:right;" format="{0,number,0,000} đ" />
								    <display:column title="First code" headerClass="transhead" class="transdata" property="commandCode" style="border: 1px solid #CCC;" />
								    ...
								    <display:column title="Time" headerClass="transhead" class="transdata" property="requestTime" format="{0,date,yyyy-MM-dd HH:mm:ss}" style="border: 1px solid #CCC;" />
								     <display:column title="CDR" headerClass="transhead" class="transdata" sortProperty="status" style="border: 1px solid #CCC;text-align:center;">
										<c:choose>
											<c:when test="${list.billingStatus == '1' }">
												<img border="0" src="<%=request.getContextPath()%>/images/tick.png" title="Charging" />
											</c:when>
											<c:otherwise>
												<img border="0" src="<%=request.getContextPath()%>/images/error.png" title="Not charging" />
											</c:otherwise>
										</c:choose>
								    </display:column>
								    <display:column title="Status" headerClass="transhead" class="transdata" sortProperty="status" style="border: 1px solid #CCC;text-align:center;">
										<c:choose>
											...
											<c:otherwise>
												<img border="0" src="<%=request.getContextPath()%>/images/error.png" title="Unsuccess" />
											</c:otherwise>
										</c:choose>
								    </display:column>
