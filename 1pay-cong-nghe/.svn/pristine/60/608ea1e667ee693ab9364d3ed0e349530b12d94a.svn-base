...
public class MoneyFlowRecordDAOMongoImpl implements MoneyFlowRecordDAO{
	...
	public List<MoneyFlowRecord> findAllMoneyFlowRecords() {
		...
		List<MoneyFlowRecord> moneyFlowRecords = mongoTemplate.find(query, MoneyFlowRecord.class);
		
		if(moneyFlowRecords != null && moneyFlowRecords.size() > 0){
			for(MoneyFlowRecord mfr : moneyFlowRecords){
	...
	public MoneyFlowRecord findMoneyFlowRecord(String id) {
		...
		MoneyFlowRecord moneyFlowRecord = mongoTemplate.findOne(query, MoneyFlowRecord.class);
	...
	public boolean update(MoneyFlowRecord moneyFlowRecord) {
	    ...
	    if (this.mongoTemplate.findOne(query, MoneyFlowRecord.class) != null) {
	    	moneyFlowRecord.setCreated_time(Utils.convertToMongoDBTime(moneyFlowRecord.getCreated_time()));
	    	moneyFlowRecord.setUpdated_time(Utils.convertToMongoDBTime(moneyFlowRecord.getUpdated_time()));
	    	moneyFlowRecord.setMonth_year(Utils.convertToMongoDBTime(moneyFlowRecord.getMonth_year()));
	...
	public List<MoneyFlowRecord> findAllMoneyFlowRecordsInRevenueAndCostType(String last_period, Date last_month_year) {
		Query query = new Query();
		query.addCriteria(Criteria.where("status").is(1));
		query.addCriteria(Criteria.where("request").is(MoneyFlowRecord.ACCEPTED));
		
		query.with(new Sort(Sort.Direction.ASC, new String[] {"type"}));
		query.with(new Sort(Sort.Direction.ASC, new String[] {"period"}));
		query.with(new Sort(Sort.Direction.DESC, new String[] {"month_year"}));
		
		query.addCriteria(Criteria.where("type").exists(true).orOperator(Criteria.where("type").is(MoneyFlowCategory.REVENUE_TYPE), Criteria.where("type").is(MoneyFlowCategory.COST_TYPE)));
	...
	public void updateCodeName(MoneyFlowCategory cate) {
		List<MoneyFlowRecord> records = findMoneyFlowRecords(cate.getId());
		
		for(MoneyFlowRecord record : records){
