						...
						<form id="dashboardForm" action="dashboard.html" method="get">
							...
							<div class="dash_row">
								<div class="other_filter">
									...
									<select  id="chargingService" name="chargingService" style="float: left; width:85px; margin-right:5px;">
										...
										<option ${param.chargingService == 'IAC'?'selected="selected"':''} value="IAC">SMS Plus Charging</option>
									</select>
									</c:if>
							...
							<div class="dash_row">
								<div class="dash_no_box">
									...
									<fmt:formatNumber value="${model.totalBillingSuccessSummary }" var="totalBillingSuccessSummaryFmt" currencyCode="vnd" minFractionDigits="0" maxFractionDigits="0"/>
									...
									<fmt:formatNumber var="totalBillingSuccessPercent" value="${(model.totalPassBillingSuccessSummary4Percent > 0 && model.totalBillingSuccessSummary4Percent > 0)?( model.totalBillingSuccessSummary4Percent/model.totalPassBillingSuccessSummary4Percent -1) * 100:0}" maxFractionDigits="2" minFractionDigits="2" />
									...
									<table cellpadding="0" cellspacing="0" width="100%" border="0">
										<tr>
											<td width="20%" class="dash_no_head">Lượt giao dịch(<span style="color:${fn:contains(totalTransPercent,'-')?'#F30':'#0C0' }"><c:out value="${totalTransPercent}"/>%</span>)</td>
											<td width="20%" class="dash_no_head">Tỉ lệ lỗi(<span style="color:${fn:contains(totalFailurePercent,'-')?'#0C0':'#F30' }"><c:out value="${totalFailurePercent}"/>%</span>)</td>
											<td width="20%" class="dash_no_head">Thành công(<span style="color:${fn:contains(totalSuccessPercent,'-')?'#F30':'#0C0' }"><c:out value="${totalSuccessPercent}"/>%</span>)</td>
											<td width="20%" class="dash_no_head">Tính cước(<span style="color:${fn:contains(totalBillingSuccessPercent,'-')?'#F30':'#0C0' }"><c:out value="${totalBillingSuccessPercent}"/>%</span>)</td>
											<td align="right">Doanh thu phân chia(đ)(<span style="color:${fn:contains(totalRevenuePercent,'-')?'#F30':'#0C0' }"><c:out value="${totalRevenuePercent}"/>%</span>)&nbsp;<img id="myTip" src="<%=request.getContextPath() %>/images/question.png" title="Chú thích" style="vertical-align: bottom; margin-right: 5px;" width="16px"/></td>
										</tr>
										<tr>
											...
											<td class="dash_no"><span id="totalBillingSuccessSummary"> <c:out value="${totalBillingSuccessSummaryFmt}" /></span></td>
											<c:choose>
							...
							<div style="color:#f00;">
								<strong>Lưu ý:</strong> Doanh thu phân chia được tính dựa trên giao dịch Tính cước thành công.
							</div>
							...
							<div class="dash_row">
								<div id="dash_tab">
									<c:if test="${model.merchants !=null && fn:length(model.merchants) > 1}">
									<a id="merchant" href="#" class="tab${(param.tab == null || param.tab=='' || param.tab=='merchant')?'_slc':''}"><span>Mer.</span></a>
									<a id="new_merchant" href="#" class="tab${param.tab=='new_merchant'?'_slc':''}"><span>Mer. mới</span></a>
									</c:if>
									...
									<c:if test="${!MBIZ}">
									<a id="service" href="#" class="tab${param.tab == 'service'?'_slc':''}"><span>D.Vụ</span></a>
									</c:if>
								...
								<script type="text/javascript">
									function drawLineVisualization() {
									  ...
									  for(var i=1; i<data.getNumberOfColumns(); i++)
										formatter.format(data, i); // Apply formatter from second column
									  var options = {
									  };
																...
																<td class="svr_data">
																	<table cellpadding="0" cellspacing="0" width="100%" border="0">
																		<tr>
																			...
																			<td>
																				<a href="<%=request.getContextPath()%>/protected/dashboard.html?${onepay:setParam(pageContext.request.queryString,'chargingService',item.chargingService) }"><strong><c:out value="${onepay:chargingServiceCode2Text(item.chargingService)}" /></strong></a>
																			</td>
																		</tr>
																	</table>
																</td>
