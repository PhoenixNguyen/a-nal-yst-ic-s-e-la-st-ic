...
<%@ page import="vn.onepay.provider.comparison.model.ProviderComparison"%>
...
<body>
						...
						<div class="right_content">
							...
							<div>
								<div id="dash_tab">
									...
									<a class="tab" href="<%=request.getContextPath() %>/protected/provider_comparison_log.html?id=${model.comparison_id}"><span>Log trạng thái</span></a>
								...
								<div>
									<c:choose>
										<c:when test="${model.comparison.contract_type == 'A_B2B' }">
											<jsp:include page="include_provider_comparison_a_b2b_detail.jsp" />
										</c:when>
										<c:when test="${model.comparison.contract_type == 'A_B2C' }">
											<jsp:include page="include_provider_comparison_a_b2c_detail.jsp" />
										</c:when>
										<c:when test="${model.comparison.contract_type == 'B1_B2B' }">
											<jsp:include page="include_provider_comparison_b1_b2b_detail.jsp" />
										</c:when>
										<c:when test="${model.comparison.contract_type == 'B1_B2C' }">
											<jsp:include page="include_provider_comparison_b1_b2c_detail.jsp" />
										</c:when>
										<c:when test="${model.comparison.contract_type == 'B2_CN' }">
											<jsp:include page="include_provider_comparison_b2_cn_detail.jsp" />
										</c:when>
										<c:when test="${model.comparison.contract_type == 'C_CN' }">
											<jsp:include page="include_provider_comparison_c_cn_detail.jsp" />
										</c:when>
										<c:otherwise>
	...
	<!-- Comparison Popup -->
		<jsp:include page="include_provider_comparison_popup.jsp"></jsp:include>
	<!-- /Comparison Popup -->
