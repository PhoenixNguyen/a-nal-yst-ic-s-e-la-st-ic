...
$(function(){
    ...
    //hasHardCopy check all
    $("#changeHasHardCopy").click(function(){
    	if($("#changeHasHardCopy").is(':checked'))
    	{
    		$('input[name=hardCopyStatus]').attr('checked', true);
    	}
    	else
    	{
    		$('input[name=hardCopyStatus]').attr('checked', false);
    	}
    });
});
</script>
...
$(function(){
    ...
    //hasHardCopy
    $('input[name=changeHasHardCopy]').attr('checked', true);
    $('input[name=hardCopyStatus]').attr('checked', true);
});
</script>
</c:if>
										...
										<div class="box_locketqua">
													...
													<select id="filter_policy" name="paymentPolicy" multiple="multiple" class="dashboard_filter">
														<c:forEach var="paymentPolicy" items="${model.paymentPolicies}">
															...
															<option value="${paymentPolicy }" ${fn:contains(allPolicies, paymentPolicy2)?'selected':'' }>
																<c:out value="${onepay:merchantPolicyCode2Text(paymentPolicy)}"/>
															</option>
														</c:forEach>
													</select>
												</div>
							                ...
							                <div class="filter_row">
							                	...
							                	<fieldset class="fieldset_filter">
													...
													<input type="checkbox" ${fn:contains(allCharging, 'IAC')?'checked="checked"':'' } title="SMS Plus" value="IAC" name="chargingService" class="chck_filter chargingService" id="chargingService"> 
													<label for="chck_wap" class="lbl_chcksub">SMS Plus</label>
												</fieldset>
                							</div>
                							
                							<div class="filter_row">
							                	<c:set var="allHasHardCopy" value="," />
												<c:forEach var="cs" items="${paramValues.hardCopyStatus}">
													<c:set var="allHasHardCopy" value="${allHasHardCopy}${cs}," />
												</c:forEach>
							                	<fieldset class="fieldset_filter">
													<legend>
									                	<span>
									                		<input type="checkbox" ${param.changeHasHardCopy!=null?'checked="checked"':'' } name="changeHasHardCopy" id="changeHasHardCopy">
									                		Tất cả:
									                	</span>
													</legend>
													<input type="checkbox" ${fn:contains(allHasHardCopy, 'true')?'checked="checked"':'' } title="Có bản cứng HĐ" value="true" name="hardCopyStatus" class="chck_filter hardCopyStatus" id="hardCopyStatus">
                                                    <label for="chck_thecao" class="lbl_chcksub">Có bản cứng HĐ</label>
                                                       
                                                    <input type="checkbox" ${fn:contains(allHasHardCopy, 'false')?'checked="checked"':'' } title="Không có bản cứng HĐ" value="false" name="hardCopyStatus" class="chck_filter hardCopyStatus" id="hardCopyStatus">
                                                    <label for="chck_sms" class="lbl_chcksub">Không có bản cứng HĐ</label>
                                                </fieldset>
                							</div>
							                ...
							                <div class="filter_row">
							                 	...
							                 	<select id="status" name="status" class="dashboard_filter">
							                 		...
							                 		<option value="0" ${(param.status == '0')?'selected="selected"':''}>0-Chưa duyệt</option>
							                 		<option value="1" ${(param.status == '1')?'selected="selected"':''}>1-Đã duyệt</option>
							                 	</select>
							                </div>
							                </c:if>
                							 -->
