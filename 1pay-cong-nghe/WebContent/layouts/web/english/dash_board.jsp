						...
						<form id="dashboardForm" action="dashboard.html" method="get">
							...
							<div class="dash_row">
								<div class="other_filter">
									...
									<select  id="chargingService" name="chargingService" style="float: left; width:85px; margin-right:5px;">
										<option value="" ${(param.chargingService == null || param.chargingService=='')?'selected="selected"':''}>Charging Services</option>
									...
									<select  id="merchantPolicy" name="merchantPolicy" style="float: left; width:90px; margin-right:5px;">
										<option value="" ${(param.merchantPolicy == null || param.merchantPolicy=='')?'selected="selected"':''}>Policy</option>
									...
									<select  id="merchantContract" name="merchantContract" style="float: left; width:90px; margin-right:5px;">
										<option value="" ${(param.merchantContract == null || param.merchantContract=='')?'selected="selected"':''}>Contract</option>
										...
										<td width="20%">
											...
											<table cellpadding="0" cellspacing="0" width="100%" border="0" >
												...
												<tr>
											    	...
											    	<fmt:formatNumber var="totalDeptRevenuePercent" value="${(model.mapDeptTotalPassRevenue4Percent[dept] > 0 && model.mapDeptTotalRevenue4Percent[dept] > 0)?( model.mapDeptTotalRevenue4Percent[dept]/model.mapDeptTotalPassRevenue4Percent[dept] -1) * 100:0}" maxFractionDigits="2" minFractionDigits="2" />
											    	<td class="dash_no">
							...
							<div class="dash_row">
								<div class="dash_no_box">
									<fmt:formatNumber value="${model.totalTransSummary }" var="totalTransSummaryFmt" currencyCode="vnd" minFractionDigits="0" maxFractionDigits="0"/>
									<fmt:formatNumber value="${model.totalFailureSummary }" var="totalFailureSummaryFmt" currencyCode="vnd" minFractionDigits="0" maxFractionDigits="0"/>
									<fmt:formatNumber value="${model.totalSuccessSummary }" var="totalSuccessSummaryFmt" currencyCode="vnd" minFractionDigits="0" maxFractionDigits="0"/>
									...
									<fmt:formatNumber var="totalTransPercent" value="${(model.totalPassTransSummary4Percent > 0 && model.totalTransSummary4Percent > 0)?(model.totalTransSummary4Percent/model.totalPassTransSummary4Percent -1) * 100:0}" maxFractionDigits="2" minFractionDigits="2" />
									<fmt:formatNumber var="totalSuccessPercent" value="${(model.totalPassSuccessSummary4Percent > 0 && model.totalSuccessSummary4Percent > 0)?( model.totalSuccessSummary4Percent/model.totalPassSuccessSummary4Percent -1) * 100:0}" maxFractionDigits="2" minFractionDigits="2" />
									<fmt:formatNumber var="totalFailurePercent" value="${(model.totalPassFailureSummary4Percent > 0 && model.totalFailureSummary4Percent > 0)?( (model.totalFailureSummary4Percent*model.totalPassTransSummary4Percent)/(model.totalPassFailureSummary4Percent*model.totalTransSummary4Percent) -1) * 100:0}" maxFractionDigits="2" minFractionDigits="2" />
									...
									<fmt:formatNumber var="failurePercent" value="${model.totalTransSummary > 0?(model.totalFailureSummary/model.totalTransSummary) * 100:0}" maxFractionDigits="2" minFractionDigits="2" />
									<table cellpadding="0" cellspacing="0" width="100%" border="0">
										<tr>
											<td width="22%" class="dash_no_head">Transactions(<span style="color:${fn:contains(totalTransPercent,'-')?'#F30':'#0C0' }"><c:out value="${totalTransPercent}"/>%</span>)</td>
											<td width="22%" class="dash_no_head">Error Rate(<span style="color:${fn:contains(totalFailurePercent,'-')?'#0C0':'#F30' }"><c:out value="${totalFailurePercent}"/>%</span>)</td>
											<td width="22%" class="dash_no_head">Successful Rate(<span style="color:${fn:contains(totalSuccessPercent,'-')?'#F30':'#0C0' }"><c:out value="${totalSuccessPercent}"/>%</span>)</td>
											<td align="right">Sharing Revenue (VND)(<span style="color:${fn:contains(totalRevenuePercent,'-')?'#F30':'#0C0' }"><c:out value="${totalRevenuePercent}"/>%</span>)&nbsp;<img id="myTip" src="<%=request.getContextPath() %>/images/question.png" title="Note" style="vertical-align: bottom; margin-right: 5px;" width="16px"/></td>
										</tr>
										<tr>
											<td class="dash_no"><span id="totalTransSummary"> <c:out value="${totalTransSummaryFmt}" /></span></td>
											<td class="dash_no"><span id="totalFailureSummary"> <c:out value="${failurePercent}" />%</span></td>
											<td class="dash_no"><span id="totalSuccessSummary"> <c:out value="${totalSuccessSummaryFmt}" /></span></td>
											<c:choose>
												<c:when test="${model.totalRevenueSummary4Percent > model.totalPassRevenueSummary4Percent}">
													<td class="dash_no_money"><span id="totalRevenueSummary"> <c:out value="${totalRevenueSummaryFmt}" /></span></td>
												</c:when>
												<c:when test="${model.totalRevenueSummary4Percent < model.totalPassRevenueSummary4Percent}">
													<td class="dash_no"><span id="totalRevenueSummary" style="color: red"> <c:out value="${totalRevenueSummaryFmt}" /></span></td>
												</c:when>
												<c:otherwise>
													<td class="dash_no"><span id="totalRevenueSummary"> <c:out value="${totalRevenueSummaryFmt}" /></span></td>
												</c:otherwise>
											</c:choose>
										</tr>
									</table>
	               					<script type="text/javascript" language="javascript">
										$(document).ready(function() {
											...
											$("#myTip").qtip({
												content: 'Sharing Revenue: Total revenue that 1Pay gets after Telcos.',										
												position : {
													corner : {
														target : 'topRight',
														tooltip : 'bottomLeft'
							...
							<div class="dash_row">
								<div id="dash_tab">
									<c:if test="${model.merchants !=null && fn:length(model.merchants) > 1}">
									<a id="merchant" href="#" class="tab${(param.tab == null || param.tab=='' || param.tab=='merchant')?'_slc':''}"><span>Merchants</span></a>
									<a id="new_merchant" href="#" class="tab${param.tab=='new_merchant'?'_slc':''}"><span>New Merchants</span></a>
									</c:if>
									<a id="cont" href="#" class="tab${(param.tab=='cont' || ( (model.merchants== null || fn:length(model.merchants) < 1) && (param.tab == null || param.tab=='')))?'_slc':''}"><span>Products</span></a>
									<c:if test="${!MBIZ}">
									<a id="service" href="#" class="tab${param.tab == 'service'?'_slc':''}"><span>Services</span></a>
									</c:if>
									...
									<a id="telco" href="#" class="tab${param.tab == 'telco'?'_slc':''}">
										<span><c:out value="${param.chargingService =='CARD'?'Card Types':'Telcos'}"></c:out></span>
									</a> 
									<a id="price" href="#" class="tab${param.tab == 'price'?'_slc':''}"><span>Prices</span></a>
									
									<c:if test="${model.isStaff && model.merchantContracts!=null && fn:length(model.merchantContracts) > 1}">
									<a id="merchant_policy" href="#" class="tab${param.tab == 'merchant_policy'?'_slc':''}"><span>Policy</span></a>
									</c:if>
									<c:if test="${model.isStaff && account_logined.isAdmin() && model.merchantContracts!=null && fn:length(model.merchantContracts) > 1}">
									<a id="merchant_contract" href="#" class="tab${param.tab == 'merchant_contract'?'_slc':''}"><span>Contract</span></a>
									</c:if>
									<div class="clear"></div>
								</div>
							...
							<div class="dash_row">
								...
								<c:if test="${(param.tab=='telco' && model.telcosPi!=null && model.telcosLine!=null && fn:length(model.telcosPi)>0 && fn:length(model.telcosLine)>0)
												...
												|| (model.merchantProviderPi!=null && fn:length(model.merchantProviderPi)>0 && model.merchantProviderLine!=null && fn:length(model.merchantProviderLine)>0)
												}">
										...
										<tr>
											<c:choose>
												<c:when test="${param.tab=='telco'}">
													<td width="360">
														<c:out value="${param.chargingService =='CARD'?'Card Type':'Telco'}"></c:out>
													</td>
												</c:when>
												<c:when test="${param.tab=='price'}">
													<td width="360">Price (VNĐ)</td>
												</c:when>
												<c:when test="${param.tab=='merchant'}">
													<td width="360">Merchant</td>
												</c:when>
												<c:when test="${param.tab=='new_merchant'}">
													<td width="360">Merchant</td>
												</c:when>
												<c:when test="${model.providers!=null && param.tab=='provider'}">
													<c:if test="${!MBIZ}">
													<td width="360">Provider</td>
													</c:if>
												</c:when>
												<c:when test="${param.tab=='merchant_policy'}">
													<td width="360">Payment</td>
												</c:when>
												<c:when test="${param.tab=='merchant_contract'}">
													<c:if test="${model.isStaff && account_logined.isAdmin()}">
													<td width="360">Contract</td>
													</c:if>
												</c:when>
												<c:when test="${param.tab=='service'}">
													<c:if test="${!MBIZ}">
													<td width="360">Service</td>
													</c:if>
												</c:when>
												<c:otherwise>
													<td width="360">Product</td>
												</c:otherwise>
											</c:choose>
											<td width="150"> Sharing Revenue (đ)</td>
									...
									<div class="table_data" id="table_data">
										...
										<table cellpadding="0" cellspacing="0" width="100%" border="0">
											<c:choose>
													...
													<c:forEach var="item" items="${model.telcosPi}" varStatus="status">
														...
														<%};%>
															...
															<td class="doanhthu_data">
																<fmt:formatNumber var="doanhthu" value="${item.totalRevenue }" currencyCode="vnd" minFractionDigits="0" maxFractionDigits="0"/>
																<c:out value="${doanhthu}" />
															</td>
															<td class="luotgd_data">
																<fmt:formatNumber var="luotgd" value="${item.totalSuccess }" currencyCode="vnd" minFractionDigits="0" maxFractionDigits="0"/>
																<c:out value="${luotgd}" />
															</td>
															<td class="tyle_data">
														...
														<%-- <c:if test="${model.itemsView==null || fn:length(model.itemsView)==0 || fn:contains(model.itemsView, itemView)}"> --%>
															<c:set var="piArr" value="${piArr}['${onepay:getOperatorName(item.telco) }', ${item.totalRevenue}]" />
															<c:if test="${!status.last}">
																...
																<td class="svr_data">
																	<table cellpadding="0" cellspacing="0" width="100%" border="0">
																		<tr>
																			...
																			<td style="padding-right: 110px;text-align: right;">
												...
												<c:when test="${(model.merchants!=null && fn:length(model.merchants)>1) && (param.tab == null || param.tab=='' || param.tab=='merchant')}">
																...
																<td class="svr_data">
																	<table cellpadding="0" cellspacing="0" width="100%" border="0">
																		<tr>
																			...
																			<td>
																				<a href="<%=request.getContextPath()%>/protected/dashboard.html?${onepay:setParam(pageContext.request.queryString,'merchant',item.merchant) }">
																					<strong>
																						<c:out value="${item.merchant }" /> 
																...
																<td class="svr_data">
																	<table cellpadding="0" cellspacing="0" width="100%" border="0">
																		<tr>
																			...
																			<td>
																				<a href="<%=request.getContextPath()%>/protected/dashboard.html?${onepay:setParam(pageContext.request.queryString,'merchant',item.merchant) }"><strong><c:out value="${item.merchant }" /> (<c:out value="${onepay:getDepartmentOfMerchant(item.merchant) }" />-<c:out value="${onepay:getContractTypeOfMerchant(item.merchant, pageContext.request) }" />)</strong></a>
																			</td>
																		</tr>
																	</table>
																</td>
												...
												<c:when test="${model.providers!=null && param.tab=='provider'}">
													...
													<c:forEach var="item" items="${model.merchantProviderPi}">
																...
																<td class="svr_data">
																	<table cellpadding="0" cellspacing="0" width="100%" border="0">
																		<tr>
																			...
																			<td>
																				<a href="<%=request.getContextPath()%>/protected/dashboard.html?${onepay:setParam(pageContext.request.queryString,'paymentProvider',item.paymentProvider) }"><strong><c:out value="${onepay:providerCode2Text(item.paymentProvider) }" /></strong></a>
																			</td>
																		</tr>
																	</table>
																</td>
															...
															<c:set var="piArr" value="${piArr}['${onepay:providerCode2Text(item.paymentProvider) }', ${item.totalRevenue}]" />
															<c:if test="${!status.last}">
																<c:set var="piArr" value="${piArr}," />
															</c:if>
													</c:forEach>
												...
												<c:when test="${model.isStaff && param.tab=='merchant_policy'}">
													<c:forEach var="item" items="${model.merchantProviderPi}">
																...
																<td class="svr_data">
																	<table cellpadding="0" cellspacing="0" width="100%" border="0">
																		<tr>
																			...
																			<td>
																				<a href="<%=request.getContextPath()%>/protected/dashboard.html?${onepay:setParam(pageContext.request.queryString,'merchantPolicy',item.merchantPolicy) }"><strong><c:out value="${onepay:merchantPolicyCode2Text(item.merchantPolicy) }" /></strong></a>
																			</td>
																		</tr>
																	</table>
																</td>
															...
															<c:set var="piArr" value="${piArr}['${onepay:merchantPolicyCode2Text(item.merchantPolicy) }', ${item.totalRevenue}]" />
															<c:if test="${!status.last}">
																<c:set var="piArr" value="${piArr}," />
															</c:if>
													</c:forEach>
												...
												<c:when test="${model.isStaff && account_logined.isAdmin() && param.tab=='merchant_contract'}">
													<c:forEach var="item" items="${model.merchantProviderPi}">
																	...
																	<table cellpadding="0" cellspacing="0" width="100%" border="0">
																		<tr>
																			<td width="13" style="padding-right: 5px;">
																				...
																				type="checkbox" 
																				${(model.itemsView==null || fn:length(model.itemsView)==0 || fn:contains(model.itemsView, itemView))?'checked="checked"':'' } 
																				value="${item.merchantContract}" class="chck_svr" /></td>
																			<td width="13" style="padding-right: 5px;">
																			</td>
																			<td>
																				<a href="<%=request.getContextPath()%>/protected/dashboard.html?${onepay:setParam(pageContext.request.queryString,'merchantContract',item.merchantContract) }"><strong><c:out value="${onepay:merchantContractCode2Text(item.merchantContract) }" /></strong></a>
																			</td>
																		</tr>
																	</table>
																</td>
															...
															<c:set var="piArr" value="${piArr}['${onepay:merchantContractCode2Text(item.merchantContract) }', ${item.totalRevenue}]" />
															<c:if test="${!status.last}">
																<c:set var="piArr" value="${piArr}," />
															</c:if>
													</c:forEach>
																...
																<td class="svr_data">
																	<table cellpadding="0" cellspacing="0" width="100%" border="0">
																		<tr>
																			...
																			<td>
																				<a href="<%=request.getContextPath()%>/protected/dashboard.html?${onepay:setParam(pageContext.request.queryString,'chargingService',item.chargingService) }"><strong><c:out value="${item.chargingService }" /></strong></a>
																			</td>
																		</tr>
																	</table>
																</td>
												...
												<c:otherwise>
													<c:forEach var="item" items="${model.contsPi}">
														...
														<%-- <c:if test="${model.itemsView==null || fn:length(model.itemsView)==0 || fn:contains(model.itemsView, itemView)}"> --%>
															<c:set var="piArr" value="${piArr}['${onepay:getContentName(item.contentService, pageContext.request) }', ${item.totalRevenue}]" />
															<c:if test="${!status.last}">
																<c:set var="piArr" value="${piArr}," />
															</c:if>
														<%-- </c:if> --%>
												...
												<tr>
													<td colspan="10" align="right" style="padding:5px;">
														<a href="#" class="show_hide" id="plus" style="float:right; padding-right:20px; cursor: pointer;">[+] Read more</a>
													</td>
												</tr>
												<%} %>
										</table>
