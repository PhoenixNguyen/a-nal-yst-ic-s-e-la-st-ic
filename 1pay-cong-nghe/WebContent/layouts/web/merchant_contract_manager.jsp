				...
				<div id="w_body">
					<div class="row_sub">
						...
						<div class="right_content">
							<h1 class="srv_title" style="width:300px; float:left;">Hợp đồng điện tử</h1>
							
							<c:if test="${model.isReviewProfileInfo}">
								<a href="<%=request.getContextPath() %>/protected/create_merchant_contract.html" class="addproduct_link"><span>Tạo hợp đồng mới</span></a>
							</c:if>
							<%if(account.checkRole(Account.ACCOUNT_STAFF_ROLE) && account.checkRoles(new String[]{Account.ACCOUNT_BIZ_SUPPORTER_ROLE})){ %>
								<a href="<%=request.getContextPath() %>/protected/export_merchant_contract.html?type=support" class="export_link" style="margin-right: 5px;"><span>Xuất TT cơ bản</span></a>
							<%} %>	
							<%if(account.checkRole(Account.ACCOUNT_STAFF_ROLE) && account.checkRoles(new String[]{Account.ACCOUNT_OPERATION_MANAGER_ROLE})){ %>
								<a href="<%=request.getContextPath() %>/protected/export_merchant_contract.html?type=operator" class="export_link" style="margin-right: 5px;"><span>Xuất TT đầy đủ</span></a>
							<%} %>	
							<div class="clear"></div>
								<div>
									...
									<form id="list-sms-scenario" action="merchant_contract_manager.html" method="post">
				                 	...
				                 	<div class="srv_row">
										...
										<display:table name="model.list" id="list" 
										    ...
										    <display:column title="Chu kỳ TT" headerClass="transhead " class="transdata" style="border: 1px solid #CCC;text-align:center;">
										    	${list.paymentPolicy}
										    </display:column>
										    ...
										    <display:column title="Actions" headerClass="transhead width40" class="svr_data1" style="border: 1px solid #CCC;text-align:center;">
										    	<a href="merchant_contract_detail.html?id=${list.id}" class="act_detail" title="Xem Merchant Profile" target="blank"></a>
										    	<!-- 
										...
										<script type="text/javascript">
													$(document).ready(function() {
														$('.onoffswitch-checkbox').live('click', function() {
															var alertMess = 'Bạn muốn thay đổi trạng thái hợp đồng không?';
															var active = $(this).attr('value')+'';
															if(active == '0') {
																alertMess = 'Bạn muốn duyệt hợp đồng?';
															} else if(active == '1') {
																alertMess = 'Bạn muốn hủy duyệt hợp đồng?';
															}
