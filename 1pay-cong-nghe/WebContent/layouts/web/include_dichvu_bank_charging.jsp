...
<div id="w_body">
	<div class="row_sub">
		<%request.setAttribute("bankCharging", true);%>
		<jsp:include page="include_dichvu_menu.jsp"></jsp:include>
		
		<div class="right_content">
			<div class="srv_details_box">
				<div class="srv_img_thumb">
					<img src="images/bank_charging-logo.jpg" border="0" />
					<c:set var="bank_charging_code" value="<%=SharedConstants.BANK_CHARGING_SERVICE_CODE %>" scope="page"/> 
					<c:choose>
						<c:when test="${account_activated_chargings!=null && account_activated_chargings[bank_charging_code]}">
							<a href="<%=request.getContextPath() %>/console/index.wss" class="btn_dangkysrv">Quản lý sản phẩm</a>
						</c:when>
						<c:otherwise>
							<a href="<%=request.getContextPath() %>/protected/bank_contract.html" class="btn_dangkysrv">Kích hoạt dịch vụ</a>
						</c:otherwise>
					</c:choose>
				</div>
				<div class="srv_details">
					...
					<div class="srv_detail_des">Dịch vụ sử dung hình thức thanh toán trên BANK, chỉ với 1 click thay vì việc phải nhớ các cú pháp phức tạp, mất thời gian để soạn tin nhắn và gửi tới đầu số..</div>
					<div class="srv_mainfeature">
						<a href="<%= request.getContextPath() %>/bank/chinh-sach.html" class="srv_feature_link"><img
							src="images/document_icon.png" border="0" hspace="5"
							align="absmiddle" />Chính sách</a> · 
							<c:choose>
								<c:when test="${account_activated_chargings!=null && account_activated_chargings[bank_charging_code]}">
									<a href="<%=request.getContextPath() %>/protected/bank_contract.html" class="srv_feature_link">
								</c:when>
								<c:otherwise>
