	...
	<div class="header-inner container">
		...
		<div style="width:195px; float:left">
			...
			<a href="index.html" style="padding: 2px;"> 
				...
				<span style=" font-size: 12px; color: #fff; line-height: 14px; padding-left: 8px; font-style: italic; width: 90px; display: inline-block; height: 30px; margin-top: 12px; "> Quick connect Pay flexible </span>
			</a>
				...
				<c:choose>
					<c:when test="${account_logined != null}">
						<li class="dropdown user">
							<a href="<%=request.getContextPath()%>/protected/profile.html" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true"> 
							...
							<ul class="dropdown-menu">
								...
								<li><a href="<%= request.getContextPath()%>/protected/cash-flow.html"><i class="fa fa-money"></i> Báo cáo LNG</a></li>
								<%} %>
								<%if(!SharedConstants.MBIZ && (account.isAdmin() || account.isOperator() || account.isBizManager())){%>
								<li><a href="<%= request.getContextPath()%>/protected/kpi_chart.html"><i class="fa fa-tasks"></i> Theo dõi kế hoạch</a></li>
								<%} %>
								<%if(!SharedConstants.MBIZ && (account.isAdmin() || account.isOperator() || account.isBizSupportManager())){%>
								<li><a href="<%= request.getContextPath()%>/cdr/sms-cdr-filter.cdr"><i class="fa fa-sitemap"></i> Thử nghiệm nội bộ</a></li>
								<%} %>
								<%
								...
								%>
								<li><a href="<%= request.getContextPath()%>/protected/accounts.html"><i class="fa fa-users"></i> Quản lý Merchant</a></li>
								<%} else if(!SharedConstants.MBIZ && account.checkRole(Account.ACCOUNT_MERCHANT_MANAGER_ROLE)){%>
								<li><a href="<%= request.getContextPath()%>/protected/merchant-manager.html"><i class="fa fa-users"></i> Quản lý Merchant</a></li>
								<%} %>
								...
								<li><a href="<%= request.getContextPath()%>/protected/merchant_profile_manager.html"><i class="fa fa-user"></i> Quản lý Merchant Profile</a></li>
								<%} %>
								...
								<li><a href="<%= request.getContextPath()%>/protected/provider_profile_manager.html"><i class="fa fa-list-alt"></i> Quản lý Provider's Profile</a></li>
								<%} %>
								<%if(account.checkRoles(new String[]{Account.ACCOUNT_ADMIN_ROLE, Account.ACCOUNT_OPERATION_MANAGER_ROLE})){%>
								<li><a href="<%= request.getContextPath()%>/protected/email-announcement.html"><i class="fa fa-envelope-o"></i> Thông báo Email</a></li>
								<%} %>
								<%if(account.checkRoles(new String[]{Account.ACCOUNT_ADMIN_ROLE, Account.ACCOUNT_OPERATION_MANAGER_ROLE})){%>
								<li><a href="<%= request.getContextPath()%>/protected/manage-email-marketing.html"><i class="fa fa-envelope"></i> Email Marketing</a></li>
								<%} %>
								<%if(account.checkRole( Account.ACCOUNT_MARKETING_MANAGER_ROLE)){%>
								<li><a href="<%= request.getContextPath()%>/protected/email-marketing.html"><i class="fa fa-envelope"></i> Email Marketing</a></li>
								<%} %>
								<li><a href="<%= request.getContextPath()%>/protected/profile.html"><i class="fa fa-cog"></i> Cấu hình</a></li>
					...
					<c:otherwise>
						<li>
							<div>
								<a style="line-height: 42px;color: #ccc;text-decoration: none;" href="<%=request.getContextPath()%>/protected/index.html"><i class="fa fa-key"></i> Đăng nhập</a>
							</div>
						</li>
					</c:otherwise>
				</c:choose>
