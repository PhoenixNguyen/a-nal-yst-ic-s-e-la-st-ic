...
<div class="row others">
	<c:forEach var="_art" items="${model.arts}" varStatus="status">
		<c:if test="${(art != null && _art.id != art.id) || (param.id != null && _art.id != param.id) }">
		<div class="col-md-4 blog-article">
			<h3><a href="<%=request.getContextPath()%>/dev/home.dev?cid=${param.cid}&sid=${param.sid}&id=${_art.id}">${_art.title}</a></h3>
			<p>
				${_art.description}
			</p>
		</div>
