...
public class WarningSMSFixRateService {
		...
		if (elasticSearchService.checkIndex(ESSmsCdrFilter.class)) {
			...
			dataList = elasticSearchService
					.search(fields, null, keywords, null, 0/* page */,
							count/* limit */, facetSize, ESSmsCdrFilter.class);
		}
	...
	private List<FilteringSMS> getTopMsisdn(List<ESSmsCdrFilter> sortedDataList,
			int limit) {
		...
		for (ESSmsCdrFilter mo : sortedDataList) {
			...
			if (index != -1) {
				...
				topMsisdn.add(new FilteringSMS(mo.getMsisdn(),
