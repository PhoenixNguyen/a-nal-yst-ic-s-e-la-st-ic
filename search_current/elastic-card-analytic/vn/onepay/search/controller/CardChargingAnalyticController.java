...
public class CardChargingAnalyticController extends AbstractProtectedController {
	...
	private int limit;
	private MaintenanceCalendarDAO maintenanceCalendarDAO;
  ...
  public void setMaintenanceCalendarDAO(MaintenanceCalendarDAO maintenanceCalendarDAO) {
    ...
    maintenanceCalendars = maintenanceCalendarDAO.findMaintenanceCalendars(account, null, null, fr, to, -1, -1);
    analyticsDetail(model, request, maintenanceCalendars, fr, to, fields, terms, keywords, facetSize, ESCardCdr.class);
  ...
  private <T> void analyticsDetail(ModelMap model, HttpServletRequest request, List<MaintenanceCalendar> maintenanceCalendars, 
    ...
    if (elasticSearchService.checkIndex(ESCardCdr.class)) {
      // Maintenance time
      for(String telco : CardConstants.ALL_CARD_TYPES){
        List<String> maintenanceTimes = new ArrayList<String>();
        for(MaintenanceCalendar maintenanceCalendar : maintenanceCalendars){
          if(maintenanceCalendar.getTelco().equalsIgnoreCase(telco) && maintenanceCalendar.getFrom_time().after(startTimeFilter) && maintenanceCalendar.getFrom_time().before(endTimeFilter)){
            maintenanceTimes.add(df.format(maintenanceCalendar.getFrom_time()));
            if(maintenanceCalendar.getEnd_time().after(endTimeFilter))
              maintenanceTimes.add(df.format(endTimeFilter));
            else
              maintenanceTimes.add(df.format(maintenanceCalendar.getEnd_time()));
          }
        }
        // Time Range
        if (maintenanceTimes != null && maintenanceTimes.size() > 0) {
          String operator = "_operator_time_range";
          keywords.put("timestamp" + operator, maintenanceTimes);
        }
        keywords.put("type" + "_operator_term", Arrays.asList(new String[]{telco}));
        
        if (maintenanceTimes != null && maintenanceTimes.size() > 0) {
          maintenanceCount += elasticSearchService.count(fields, terms, keywords, facetSize, ESCardCdr.class);
          //System.out.println("Bảo trì: " + maintenanceCount );
        }
      }
    }
    
    //System.out.println(" Bảo trì: " + maintenanceCount);
    model.put("maintenanceCount", maintenanceCount);
  }
