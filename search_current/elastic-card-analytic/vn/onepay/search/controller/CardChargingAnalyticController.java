...
import vn.onepay.charging.card.model.ChargeStatus;
...
public class CardChargingAnalyticController extends AbstractProtectedController {
	...
	private int limit;
	private ErrorConfigDAO errorConfigDAO;
...
//			model.put("to_date", df2.format(new Date()));
    ...
    if(tab.equals("error_detail")){
      List<ErrorConfig> errorConfigs = null;
      errorConfigs = errorConfigDAO.findErrorConfigs(account, null, null, fr, to, -1, -1);
      
      analyticsDetail(model, request, errorConfigs, fr, to, fields, terms, keywords, facetSize, ESCardCdr.class);
    }
	...
	@SuppressWarnings("unchecked")
  private <T> void analyticsDetail(ModelMap model, HttpServletRequest request, List<ErrorConfig> errorConfigs, 
      Date startTimeFilter, Date endTimeFilter,
      List<String> fields, List<String> terms,
      Map<String, List<String>> keywords, int facetSize, Class<T> clazz) {
    ...
    // Status
    if(ChargeStatus.ALL_CHARGING_ERROR_STATUS.size() > 0){
      String operator = "_operator_in";
      keywords.put("status" + operator, ChargeStatus.ALL_CHARGING_ERROR_STATUS);
    }
    
    if (elasticSearchService.checkIndex(ESCardCdr.class)) {
      //All
      // Operator time range:
      List<String> timeRanges = Arrays.asList(new String[] { df.format(startTimeFilter), df.format(endTimeFilter) });
      if (timeRanges != null && timeRanges.size() > 0) {
        String operator = "_operator_time_range";
        keywords.put("timestamp" + operator, timeRanges);
      }
      countAll = elasticSearchService.count(fields, terms, keywords, facetSize, ESCardCdr.class);
      
      // Subjective time
      for(String telco : CardChargingErrorConfigController.telcos){
        List<String> subjectiveTimes = new ArrayList<String>();
        for(ErrorConfig errorConfig : errorConfigs){
          if(errorConfig.getTelco().equalsIgnoreCase(telco) && errorConfig.getFrom_time().after(startTimeFilter) && errorConfig.getFrom_time().before(endTimeFilter)){
            subjectiveTimes.add(df.format(errorConfig.getFrom_time()));
            if(errorConfig.getEnd_time().after(endTimeFilter))
              subjectiveTimes.add(df.format(endTimeFilter));
            else
              subjectiveTimes.add(df.format(errorConfig.getEnd_time()));
          }
        }
        // Time Range
        if (subjectiveTimes != null && subjectiveTimes.size() > 0) {
          String operator = "_operator_time_range";
          keywords.put("timestamp" + operator, subjectiveTimes);
        }
        keywords.put("type" + "_operator_term", Arrays.asList(new String[]{telco}));
        
        if (subjectiveTimes != null && subjectiveTimes.size() > 0) {
          subjectiveCount += elasticSearchService.count(fields, terms, keywords, facetSize, ESCardCdr.class);
          System.out.println("Chủ quan: " + subjectiveCount );
        }
      }
      
    }
    
    objectiveCount = countAll - subjectiveCount;
    
    System.out.println("Chủ quan: " + subjectiveCount + " Khách quan: " + objectiveCount);
    
    if(subjectiveCount > 0 || objectiveCount > 0){
      Map<String , Integer> outputErrorDetail = new LinkedMap();
      outputErrorDetail.put("Lỗi chủ quan", subjectiveCount);
      outputErrorDetail.put("Lỗi khách quan", objectiveCount);
      
      model.put("outputErrorDetail", outputErrorDetail);
    }
    ...
    model.put("totalError", errorConfigs.size());
    model.put("pagesizeError", Integer.valueOf(this.limit));
    model.put("offsetError", Integer.valueOf(offset));
    model.put("listError", errorConfigs);
